...
async function login (fingerprint) {
    ...
    if(-1 !== rec_obj.device.indexOf(thisfpr)) {
        await set_new_record(kns.merge(old_record));
        runtime.emit('account.login.success');
        setImmediate(async ()=>await publish(rec_obj.device));
        return true;
    }
    // 本地有签发能力
...
async function add_device (fingerprint) {
    ...
    setImmediate(async ()=>await publish(new_obj.device));
}
 ...
 * @returns {boolean}
 */
function has_device (fingerprint) {
    return obj.account.object
        && obj.account.object.device instanceof Array
        && -1 !== obj.account.object.device.indexOf(fingerprint.toUpperCase());
}
...
async function remove_device (fingerprint) {
    ...
    setImmediate(async ()=>await publish(devices));
}
...
async function handle_new_record (r) {
    ...
    const before = status.wait;

    await set_new_record(r);
    if (before && status.login) {
