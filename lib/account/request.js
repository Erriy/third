...
async function send (devices) {
    runtime.logger.debug('[account.login.request.send] 发送到其他终端', devices);
    let success_count = 0;
    ...
    runtime.logger.debug('[account.login.request.send] success_count ',success_count);
    if(success_count === 0) {
...
async function handle_new_request (expire, d) {
