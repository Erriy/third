...
class record {
    ...
    constructor (key) {
        this.#key = key;
        this.#name = os.hostname() + (process.env.DEBUG ? '.debug' : '');
        this.#belong = key.getFingerprint().toUpperCase();
    }
    ...
    get device () {
        return {
            remove: (fpr)=>{
                fpr = fpr.toUpperCase();
                const i = this.#device.indexOf(fpr);
                if(-1 !== i) {
            ...
            add: (fpr)=>{
                fpr = fpr.toUpperCase();
                if(-1 === this.#device.indexOf(fpr)) {
                    this.#device.push(fpr);
                }
                this.#changed = true;
            },
            has: (fpr)=>{
                fpr = fpr.toUpperCase();
                return -1 !== this.#device.indexOf(fpr);
            },
            list: ()=>{
                return this.#device;
            }
    ...
    async refresh () {
        this.#signed = await openpgp.sign({
            message: await openpgp.createCleartextMessage({
                text: JSON.stringify({
                    ...
                    name    : this.#name,
                    device  : this.#device.length > 0 ? this.#device : undefined,
        ...
        runtime.emit('kns.record.refresh');
    }

    async valueOf () {
        if(!this.#signed || this.#changed || new Date().getTime() / 1000 - this.#created > 60 * 2) {
            await this.refresh();
        }
        return {
            pubkey: this.#key.toPublic().armor(),
...
async function analysis (r) {
    ...
    const o = JSON.parse(verified.data);
    ...
    let expire = created + 60 * 5;
    if(o.expire) {
        try {
            const oexp = new Date(o.expire * 1000).getTime() / 1000;
...
async function store (record, local_service = undefined) {
    ...
    if(undefined === local_service) {
        await runtime.db.run(`
            ...
            values ($fingerprint, $text, $provider, $signature, $created, $expire, $pubkey)
...
/**
 * 返回本地设备记录
  * @returns {Promise<null|Array.<{fingerprint: string, text: string, provider: boolean, signature: string, pubkey: string, created: number, expire: number, local_service: null|string}>>}
 */
async function local () {
    return (await runtime.db.all(
        'select * from record where local_service not null'
    )).map(r=>({
        fingerprint  : r.fingerprint,
        text         : r.text,
        provider     : r.provider,
...
async function init ({
    ...
    // 创建本机的ticket对象
    const belong = await runtime.kv.get('account.fingerprint');
    obj.record = new record(runtime.key);
    obj.record.provider = provider;
    obj.record.belong = belong || runtime.key.getFingerprint().toUpperCase();
