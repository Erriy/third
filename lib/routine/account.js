...
async function remove_request (fingerprint) {
    await runtime.db.run('delete from login_request where fingerprint = ?', fingerprint);
    emit('login.request');
}

async function list_request (limit = 5) {
    return (await runtime.db.all(
        'select fingerprint from login_request where expire > ? order by expire desc limit ?',
