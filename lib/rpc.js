...
async function do_request (r, encrypted) {
    ...
    if(r.local_service) {
        ...
        try {
            ...
            runtime.logger.debug(`[rpc.invoke] -> ${r.fingerprint} 本地请求完成`);
            return resp.data;
        }
        catch (err) {
            runtime.logger.debug(`[rpc.invoke] -> ${r.fingerprint} 本地连接失败，清除本地服务缓存`);
    ...
    runtime.logger.debug(`[rpc.invoke] -> ${r.fingerprint} 尝试连接远程服务`);
    try {
        const resp = await axios({
            method : 'POST',
        ...
        runtime.logger.debug(`[rpc.invoke] -> ${r.fingerprint} 远程请求完成`);
        return resp.data;
    }
    catch (err) {
...
async function invoke (fingerprint, name, ...args) {
    ...
    if(!r) {
        runtime.emit('rpc.invoke.kns.failed', fingerprint);
        return;
    }
...
async function handle (encrypted) {
    ...
    let results = [];
    for(let handler of obj.handler_map[r.name]) {
    ...
    return await encrypt_and_sign(JSON.stringify({result: results}), d.pubkey);
}
