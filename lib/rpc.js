...
async function do_request (r, encrypted) {
    ...
    const error_response = (response)=>{
        return new Promise((resolve, reject)=>{
            const dl = [];
            response.data.on('data', d=>{dl.push(d);});
            response.data.on('end', ()=>{
                const j = Buffer.concat(dl).toString();
                try {
                    resolve(JSON.parse(j).message);
                }
                catch(e) {
                    reject('返回数据无法解析');
                }
    ...
    if(r.local_service) {
        ...
        try {
            ...
            if(e.response) {
                throw new Error(await error_response(e.response));
            }
...
async function invoke (fingerprint, name, ...args) {
    ...
    const nonce = uuid.v1().split('-').slice(0, -1).join('');
    let request_data = {name, args, aes256: aes.valueOf(), nonce};
    try {
...
async function handle (encrypted) {
    ...
    if(new Date().getTime() / 1000 - d.time.sign > 60) {
        throw new Error('签名超时，认定为重放数据，拒绝执行');
    }
    ...
    if(typeof r.nonce !== 'string' || r.nonce.length === 0) {
        throw new Error('nonce不存在，无法验证是否是重放数据，拒绝执行');
    }

    const nonce = d.fingerprint + '.' + r.nonce;
    try {
        await runtime.db.run(
            'insert into nonce(nonce, expire) values (?,?)',
            nonce, d.time.recv + 60 * 2
        );
    }
    catch(e) {
        throw new Error('检测到重放数据，拒绝执行');
    }
    ...
    // 如果不是流，转化成流
    if(!(result && typeof result === 'object' && typeof result.pipe === 'function')) {
...
async function init () {
    // nonce 防重放攻击
    await runtime.db.run(`create table if not exists nonce(
    ...
    const auto_delete = async ()=>{
        await runtime.db.run(`
            delete from nonce where expire < ?
