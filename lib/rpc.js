...
async function decrypt_and_verify (encrypted, pubkey) {
    ...
    return {
        ...
        from  : pubkey.getFingerprint().toUpperCase(),
        data  : d.data,
        pubkey: pubkey.armor()
    };
}

async function invoke (fingerprint, name, data = undefined) {
    if(!fingerprint || !name) {
        throw new Error('必须指定指纹和name');
    }
    let request_data = {name, data};
    try {
...
async function handle (encrypted) {
    ...
    let result = obj.handler_map[r.name](d);
    if(result instanceof Promise) {
        result = await result;
    }

    return await encrypt_and_sign(JSON.stringify(result), d.pubkey);
}

/**
 * @typedef {function({time: {sign: number, recv: number}, from: string, data: string, pubkey: string})} rpc_handler_callback
 */
