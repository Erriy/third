...
async function init ({
    ...
    server.on('request', (request, response) => {
        runtime.logger.info('[http]', request.socket.remoteAddress, request.method, request.originalUrl, response.statusCode);
    });
    ...
    return new Promise((resolve, reject) => {
        const failed_return = (e)=> {
            ...
            if(e.syscall === 'listen' && e.code === 'EADDRINUSE') {
                msg = '端口被占用，无法启动服务';
                runtime.logger.error('[http]', msg);
            }
            else {
                runtime.logger.error('[http]', e);
            }
            return reject(msg);
        };
        ...
        server.listen(runtime.port, '0.0.0.0', async ()=>{
            ...
            runtime.logger.info('[http] 服务已启动');
