...
async function publish_new_account_ticket (
    ...
    const all_devices = Array.from(new Set([...obj.account.object.device, ...devices]));
    // 推送到其他客户端，包括删除的终端和新增的终端
    await Promise.all(all_devices.map(async fpr=>{
        if(fpr === this_fpr) {
            return;
        }
...
async function send_login_request (ticket_object) {
    ...
    await Promise.all(ticket_object.device.map(async dev_fpr=>{
        const t = await ticket.get(dev_fpr, {discover: true});
        if(!t) return;
        const service = t.local_service || JSON.parse(t.text).service;
        if(!service) return;
        try {
            await axios.put(urljoin(service, 'api/account/request'), request_data);
            success_count += 1;
        }
        catch(e) {}
    }));
...
async function login (fingerprint) {
    ...
    if(fprs.length > 0) {
        ...
        await publish_new_account_ticket(ticket_object);
    }
    else {
        // 本地不具备签发能力
        if(!at) {
...
async function add_device (fingerprint) {
    ...
    await publish_new_account_ticket(obj.account.object);
}
