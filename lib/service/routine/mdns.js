...
async function init () {
    ...
    obj.mdns.on('response', async response=>{
        ...
        await Promise.all(ans.map(async a=>{
            ...
            try {
                // 保存设备ticket
                const resp = await axios.get(urljoin(third_service, 'api/ticket'), {timeout: 1000});
            ...
            try {
                // 保存 account ticket
                const resp = await axios.get(urljoin(third_service, 'api/account/ticket'), {timeout: 1000});
