 ...
 * @returns {{fingerprint: string, expire: number, created: number, object: Object, pubkey:string}}
 */
async function analysis (t) {
    ...
    return {fingerprint, expire, created, object: obj, pubkey: t.pubkey};
}
...
async function init ({
    ...
    // 自动发布托管ticket
    await publish();
    setInterval(async ()=>{await publish();}, 1000 * 60);
